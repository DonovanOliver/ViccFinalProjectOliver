 ...
 */
public class Observers extends SimEntity {
    ...
    public Observers(List<PowerHost> hosts) {
        this(hosts, DEFAULT_DELAY);
    }

    public Observers(List<PowerHost> hosts, float delay) {
        super("Observers");
        this.hosts = hosts;
        this.delay = delay;
    }
    ...
    * In practice: create a custom event (here it is called OBSERVE) with a unique int value and deal with it.
    ...
    public void startEntity() {
        ...
        for(PowerHost host:hosts){
        	Log.printLine("host("+host.getId()+") availableMips="+host.getAvailableMips());
        }
